...
class Trader:
    ...
    def calculate_rolling_stats(self, product, window=2):
        """Calculate rolling mean and standard deviation for a product's price history"""
        if product not in self.market_history:
            return None, None
        
        # Get price history
        prices = pd.Series(self.market_history[product]["mid_prices"])
        
        # Filter out invalid prices (-1)
        prices = prices[prices != -1]
        
        # Need at least window values to calculate
    ...
    def calculate_z_score(self, product, window=2):
        """Calculate z-score based on rolling statistics"""
        # Get current price
        if product not in self.market_history or len(self.market_history[product]["mid_prices"]) == 0:
            return None
        
        current_price = self.market_history[product]["mid_prices"][-1]
        if current_price == -1:  # Invalid price
            return None
        
        # Get rolling stats
        rolling_mean, rolling_std = self.calculate_rolling_stats(product, window)
    ...
    def fast_mean_reversion_strategy(self, state: TradingState, product="SQUID_INK", window=10, entry_z=0.75, exit_z=0.1):
        position = state.position.get(product, 0)
        order_depth = state.order_depths[product]
        orders = []

        mid_prices = self.market_history[product]['mid_prices']  # List of recent mid prices
        if len(mid_prices) < window:
            return orders  # Not enough history yet

        # --- EMA Calculation ---
        prices = np.array(mid_prices[-window:])
        ema = self.ema(prices, span=window)
        current_price = mid_prices[-1]
        residual = current_price - ema

        # --- Z-score of residuals ---
        mean_res = np.mean(prices - ema)
        std_res = np.std(prices - ema)
        z_score = (residual - mean_res) / (std_res + 1e-6)  # Avoid divide-by-zero

        # --- Order book info ---
        best_ask = min(order_depth.sell_orders.keys(), default=None)
        best_bid = max(order_depth.buy_orders.keys(), default=None)
        max_pos = self.LIMIT[product]
        ...
        if position == 0:
            if z_score < -entry_z:
                volume = min(max_pos, order_depth.sell_orders[best_ask] * -1)
                orders.append(Order(product, best_ask, volume))  # Buy
            elif z_score > entry_z:
                volume = min(max_pos, order_depth.buy_orders[best_bid])
                orders.append(Order(product, best_bid, -volume))  # Sell

        # --- Exit signals ---
        elif position > 0 and z_score >= -exit_z:
            volume = min(position, order_depth.buy_orders[best_bid])
            orders.append(Order(product, best_bid, -volume))  # Sell to exit

        elif position < 0 and z_score <= exit_z:
            volume = min(-position, -order_depth.sell_orders[best_ask])
            orders.append(Order(product, best_ask, volume))  # Buy to exit
    ...
    def order_flow_based_strategy(self, state: TradingState, product="SQUID_INK", window=10, volume_factor=2.0):
        position = state.position.get(product, 0)
        order_depth = state.order_depths[product]
        orders = []

        # Extract total volume on the bid and ask sides (sum all volumes)
        ...
        if best_bid is None or best_ask is None:
            return orders  # No valid market data

        # Calculate the price move relative to the average of the bid-ask spread
        mid_price = (best_ask + best_bid) / 2
        price_move = mid_price - self.market_history[product]['mid_prices'][-1]

        # Entry logic based on total volume imbalance
        if position == 0:
            if total_bid_volume > total_ask_volume * volume_factor:  # Strong buy pressure
                orders.append(Order(product, best_bid, total_ask_volume))  # Buy
            elif total_ask_volume > total_bid_volume * volume_factor:  # Strong sell pressure
                orders.append(Order(product, best_ask, -total_bid_volume))  # Sell

        # Exit logic based on price moving against position
        elif position > 0:
            if price_move < 0:  # Price goes down
                orders.append(Order(product, best_ask, -position))  # Sell to exit

        elif position < 0:
            if price_move > 0:  # Price goes up
                orders.append(Order(product, best_bid, position))  # Buy to exit

        return orders

    def squid_strategy(self, state: TradingState, product="SQUID_INK", window=10, deviation = 4):
        
        past_prices = self.market_history[product]["mid_prices"]
        ...
        if best_bid > past_prices[0] + deviation:
            return [Order(product, best_ask, 10)]
        elif best_ask < past_prices[0] - deviation:
            return [Order(product, best_bid, -10)]
        else:
            return []
    ...
    def run(self, state: TradingState):
        ...
        """
        squid = self.squid_strategy(state, window = 2, deviation = 3)
