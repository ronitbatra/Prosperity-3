...
class Trader:
    ...
    def calculate_ema(self, product: str, value_type: str = "mid_prices", lookback: int = 10):
        values = self.market_history['price_data'][product][value_type][-lookback:]
        filtered = []
        prev_valid = None
    ...
    def lookback_vwap(self, product: str, value_type: str = "vwap_prices", lookback = 10):
        prices = self.market_history['price_data'][product][value_type][-lookback:]
        volumes = self.market_history['price_data'][product]["total_volumes"][-lookback:]
        filtered = []
    ...
    def update_history(self, product: str, state: TradingState):
        if 'price_data' not in self.market_history:
            self.market_history['price_data'] = {}
        if product not in self.market_history['price_data']:
            self.market_history['price_data'][product] = {
                "mid_prices": [],
                "vwap_prices": [],
                "total_volumes": [],
                "positions": [],
        ...
        if product in state.order_depths and len(state.order_depths[product].sell_orders) > 0 and len(state.order_depths[product].buy_orders) > 0:
            ...
            self.market_history['price_data'][product]["mid_prices"].append(mid_price)
            total_value, total_volume = self.calculate_vwap(product, state.order_depths[product])
            vwap_price = total_value/total_volume
            self.market_history['price_data'][product]["vwap_prices"].append(vwap_price)
            self.market_history['price_data'][product]["total_volumes"].append(total_volume)

        else:
            self.market_history['price_data'][product]["mid_prices"].append(-1)
            self.market_history['price_data'][product]["vwap_prices"].append(-1)
            self.market_history['price_data'][product]["total_volumes"].append(0)

        self.market_history['price_data'][product]["positions"].append(state.position.get(product, 0))
        self.market_history['price_data'][product]["timestamps"].append(state.timestamp)
    ...
    def get_max_units(self, state: TradingState, target: dict, current_net: dict, limits: dict = None):
        """
        Compute the maximum number of whole units tradable for the target,
        accounting for capacity (based on position limits and adverse exposure)
        and liquidity across all order book levels.
        
        For a buy (target > 0):
          - effective_exec = max(current_position, 0)
          - effective_net  = max(current_net, 0)
          - capacity = limit - (effective_exec + effective_net)
          - liquidity = (sum of all sell order volumes) - effective_exec
        For a sell (target < 0):
          - effective_exec = max(-current_position, 0)
          - effective_net  = max(-current_net, 0)
          - capacity = limit - (effective_exec + effective_net)
          - liquidity = (sum of all buy order volumes) - effective_exec
        
        Returns the minimum units (floored) across products.
        """
        max_units_possible = float("inf")
        for product, qty in target.items():
            order_depth = state.order_depths[product]
            cur_exec = state.position.get(product, 0)
            net = current_net.get(product, 0)
            limit = limits[product] if limits and product in limits else self.LIMIT[product]
            
            if qty > 0:
                effective_exec = max(cur_exec, 0)
                effective_net = max(net, 0)
                capacity = limit - (effective_exec + effective_net)
                total_sell_liq = sum(abs(vol) for vol in order_depth.sell_orders.values())
                effective_liquidity = max(total_sell_liq - effective_exec, 0)
            elif qty < 0:
                effective_exec = max(-cur_exec, 0)
                effective_net = max(-net, 0)
                capacity = limit - (effective_exec + effective_net)
                total_buy_liq = sum(vol for vol in order_depth.buy_orders.values())
                effective_liquidity = max(total_buy_liq - effective_exec, 0)
            else:
                capacity = float("inf")
                effective_liquidity = float("inf")
            
            units_capacity = capacity // abs(qty) if abs(qty) > 0 else float("inf")
            units_liquidity = effective_liquidity // abs(qty) if abs(qty) > 0 else float("inf")
            max_units_for_product = min(units_capacity, units_liquidity)
            max_units_possible = min(max_units_possible, max_units_for_product)
        
        return 0 if max_units_possible == float("inf") else int(max_units_possible)


    def update_net_orders(self, current_net: dict, target: dict, units: int):
        
        for product, qty in target.items():
            current_net[product] = current_net.get(product, 0) + qty * units
        return current_net

    def clear_spread_orders(self, state: TradingState, spread_key: str, current_net: dict):
        """
        Clear any outstanding spread orders by inverting the stored spread target,
        computing the available units (using the updated get_max_units), updating current_net,
        and preparing clearing orders across all price levels.
        
        Returns the updated current_net.
        """
        if spread_key not in self.market_history or not self.market_history[spread_key]:
            return current_net
        stored_spread = self.market_history[spread_key]
        # Invert to form the clearing target.
        clearing_target = {product: -volume for product, volume in stored_spread.items()}
        units = self.get_max_units(state, clearing_target, current_net)
        scaled_target = {p: clearing_target[p] * units for p in clearing_target}
        for product, vol in scaled_target.items():
            current_net[product] = current_net.get(product, 0) + vol
        # Clear the stored spread after updating.
        self.market_history[spread_key] = {}
        return current_net


    def basket_orders(self, state: TradingState, zscore_entry: int, zscore_exit: int, lookback: int, current_net: dict):
        ...
        if "spread_diff_history" not in self.market_history:
        ...
        if len(past_spreads) < lookback:
            # Insufficient history; no orders to update.
            return current_net
    
        std = np.std(past_spreads)
        mean = 30  # (This can be adjusted or computed as needed.)
        ...
        if z_score >= zscore_entry:
            ...
            return self.clear_spread_orders(state, "current_spread", current_net)
        else:
            return current_net
    
        units = self.get_max_units(state, target, current_net)
        # Save the scaled target in market_history for potential clearing.
        self.market_history["current_spread"] = {p: target[p] * units for p in target}
        current_net = self.update_net_orders(current_net, target, units)
        return current_net
    
    def jam_basket2_spread(self, state: TradingState, zscore_entry, zscore_exit, lookback: int, current_net: dict):
        """
        Compute jam spread orders and update the external net orders (current_net).
        When current_net is all zero, the resulting net orders produced here should be the same as
        the legacy jam_spreads orders.

        This function uses the liquidity check from get_max_units and incorporates the executed positions,
        outstanding orders (current_net), and order book liquidity.
        """
        # Define hedge ratios and max position limits for this spread.
        basket_jam_hedge = 3.705     # Hedge ratio from baskets to jams.
        jb_max_j = 148             # Position limit for JAMS in this spread.
        jb_max_b = 40              # Position limit for PICNIC_BASKET2 in this spread.

        # Calculate relevant market prices.
        basket2_price = self.calculate_fair_value(Product.PICNIC_BASKET2, state.order_depths[Product.PICNIC_BASKET2])
        jam_price = self.calculate_fair_value(Product.JAMS, state.order_depths[Product.JAMS])
        croissant_price = self.calculate_fair_value(Product.CROISSANTS, state.order_depths[Product.CROISSANTS])
        ...
        # Store spread information in market_history for debugging and possible clearing.
        if "jam_basket2_spread" not in self.market_history:
            self.market_history["jam_basket2_spread"] = []
        self.market_history["jam_basket2_spread"].append(spread1)

        # Ensure we have enough historical data (lookback window) before acting.
        past_jb = self.market_history["jam_basket2_spread"][-lookback:]
        if len(past_jb) < lookback:
            return current_net

        # Compute z-score using the basket-to-jam spread (spread1); these values are fixed as in your legacy code.
        mean_jb = 5975
        std_jb = 63.9
        std_jb = np.std(past_jb)
        z_score_jb = (spread1 - mean_jb) / std_jb

        # Determine target orders based on the z-score.
        if z_score_jb >= zscore_entry:
            ...
            return self.clear_spread_orders(state, "current_jb_spread", current_net)
        else:
            return current_net

        # Define specific limits for this strategy leg.
        limits = {"PICNIC_BASKET2": jb_max_b, "JAMS": jb_max_j}

        # Determine the maximum units that can be traded given the target, current_net, liquidity and limits.
        units = self.get_max_units(state, target, current_net, limits)
        # Record the scaled target in market_history for potential clearing later.
        self.market_history["current_jb_spread"] = {p: target[p] * units for p in target}
        # Update the current_net dictionary with this new trade volume.
        current_net = self.update_net_orders(current_net, target, units)
        return current_net
    ...
    def run(self, state: TradingState):
        ...
        final_orders[Product.RAINFOREST_RESIN] = resin

        kelp = self.kelp_orders(state)
        final_orders[Product.KELP] = kelp

        # Initialize external net orders.
        current_net = {product: 0 for product in self.LIMIT}
        # Process strategies sequentially.
        current_net = self.basket_orders(state, zscore_entry=6, zscore_exit=4, lookback=100, current_net=current_net)
        current_net = self.jam_basket2_spread(state, zscore_entry=9, zscore_exit=2, lookback=100, current_net=current_net)

        # Convert aggregated net orders in current_net into final orders across all levels.
        for product, net in current_net.items():
            if net > 0:
                remaining = round(net)
                levels = sorted(state.order_depths[product].sell_orders.items(), key=lambda x: x[0])
                orders_list = []
                for price, avail in levels:
                    if remaining <= 0:
                        break
                    fill = min(remaining, abs(avail))
                    orders_list.append(Order(product, price, fill))
                    remaining -= fill
                if orders_list:
                    final_orders[product] = orders_list
            elif net < 0:
                remaining = round(abs(net))
                levels = sorted(state.order_depths[product].buy_orders.items(), key=lambda x: x[0], reverse=True)
                orders_list = []
                for price, avail in levels:
                    if remaining <= 0:
                        break
        ...
        for product in self.market_history['price_data']:
            for key in self.market_history['price_data'][product]:
                self.market_history['price_data'][product][key] = self.market_history['price_data'][product][key][-self.history_window:]        # Optionally truncate history arrays.
        
        if "spread_diff_history" in self.market_history:
            self.market_history["spread_diff_history"] = self.market_history["spread_diff_history"][-self.history_window:]
        if "jam_basket2_spread" in self.market_history:
        ...
        logger.flush(state, final_orders, 0, "")
        return final_orders, 0, jsonpickle.encode(traderObject)
