...
class Trader:
    ...
    def get_max_units(self, state: TradingState, target: dict):
    ...
    def place_orders(self, state: TradingState, target: dict, max_units: int):
        ...
        for product, quantity in target.items():
            ...
            if volume > 0:
                ...
                orders[product] = [Order(product, best_ask, volume)]
            else:
                best_bid = max(state.order_depths[product].buy_orders.keys())
                orders[product] = [Order(product, best_bid, volume)]
        
        return orders

    def clear_spread_orders(self, state: TradingState) -> dict:
        
        if "current_spread" not in self.market_history or not self.market_history["current_spread"]:
            return {}
        
        
        current_spread = self.market_history["current_spread"]
        ...
        self.market_history["current_spread"] = scaled_target
        ...
        self.market_history["current_spread"] = {}
    ...
    def basket_orders(self, state: TradingState, zscore_entry: int, zscore_exit: int, lookback: int):
        ...
        if(z_score >= zscore_entry):
            ...
            return self.clear_spread_orders(state)
        else:
            return {}
    ...
    def run(self, state: TradingState):
        ...
        basket = self.basket_orders(state, 6, 4, 100)
        result =result | basket

        for product in self.market_history:
            if product == "current_spread" :
                continue
            elif product == "spread_diff_history":
                self.market_history[product] = self.market_history[product][-self.history_window:]
                continue
